...
def transform_with_bins(num_array, discretizer):
    """
    Apply precomputed global bins while preserving shape.
    """
    num_array = np.array(num_array)

    original_shape = num_array.shape

    num_array_flat = num_array.reshape(-1, 1)
    binned = discretizer.transform(num_array_flat).astype(int)

    # reshape back
    binned = binned.reshape(original_shape)
...
def booleanize(binned_data, n_bins=5):
    ...
    binned_array = binned_data.to_numpy()

    booleanized_data = []

    for bin_value in range(n_bins):
        boolean_column = (binned_array == bin_value).astype(int)
        booleanized_data.append(boolean_column)

    booleanized_data = np.concatenate(booleanized_data, axis=1)

    return pd.DataFrame(booleanized_data)
