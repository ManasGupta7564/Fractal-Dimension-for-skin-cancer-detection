...
def train_model(X_train, Y_train, X_test, Y_test, features, num_classes, s, num_epochs,result_file):
    ...
    X_te = np.load(X_test, allow_pickle=True)
    Y_te = np.load(Y_test, allow_pickle=True)
    #tm_update
    ...
    for epoch in range(num_epochs):
        ...
        if (epoch + 1) % 5 == 0:
            ...
            for i in range(X_tr.shape[0]):
                Xi_train = (ctypes.c_int * features)(*X_tr[i].tolist())
                class_sums = [lib.tm_score(tm, Xi_train) for tm in tmachines]
                pred = int(np.argmax(class_sums))

                if pred == Y_tr[i]:
                    correct_train += 1

            train_acc = correct_train / len(Y_tr)
            train_acc_list.append(train_acc)

            msg = f"Epoch {epoch+1} | Train Acc: {train_acc*100:.2f}%"
            print(msg)
            ...
            for i in range(X_te.shape[0]):
                Xi_test = (ctypes.c_int * features)(*X_te[i].tolist())
            ...
            msg = f"Epoch {epoch+1} | Test Acc: {acc*100:.2f}%"
            print(msg)
            result_file.write(msg + "\n")
            result_file.flush()
            ...
            if acc > best_acc:
                best_acc = acc
                msg = f"🔥 Best so far: {best_acc*100:.2f}%"
                print(msg)
                result_file.write(msg + "\n")
                result_file.flush()
...
def save_model(lib, tmachines, feat, clause, save_dir, ta_dir):
    ...
    path = os.path.join(ta_dir, f"TA_{feat}_{clause}.txt")
...
def predict(lib, tmachines, X_test, features, num_classes, cls_sm_dir):
...
def evaluate_model(predicted_labels, true_labels, result_file):
    ...
    precision = precision_score(true_labels, predicted_labels, average=avg, zero_division=0)
    recall = recall_score(true_labels, predicted_labels, average=avg, zero_division=0)
    f1 = f1_score(true_labels, predicted_labels, average=avg, zero_division=0)
...
def main():
    ...
    parser.add_argument("--ta_dir", type=str, default="./TA_STATE")
    parser.add_argument("--cls_dir", type=str, default="./Class_Sums")
    parser.add_argument('--result_file', type=str, default='Result.txt')
    args = parser.parse_args()
    result_file = open(args.result_file, "w")
    # Header
    result_file.write("===== Tsetlin Machine Training =====\n")
    result_file.write(f"X_train: {args.X_train}\n")
    result_file.write(f"X_test: {args.X_test}\n")
    result_file.write(f"CLAUSES: {args.clauses}\n")
    result_file.write(f"CLASSES: {args.num_classes}\n")
    result_file.write(f"FEATURES: {args.features}\n")
    
    result_file.write(f"s: {args.s}\n")
    result_file.write(f"Epochs: {args.num_epochs}\n")
    result_file.write("\n")
    result_file.flush()
    ...
    # Save model
    save_model(lib, tmachines, args.features, args.clauses, args.save_dir, args.ta_dir)
    ...
    predicted_labels, res = predict(lib, tmachines, X_te, args.features, args.num_classes, args.cls_dir)
    evaluate_model(predicted_labels, Y_te,result_file)
    result_file.close()
